...
let rec resolve value env =
  match value with
  | Name n ->
      (match env_lookup n env with Some v -> v | None -> Error)
  | v -> v
  ...
  let rec loop stack stack_env env cmds =
    ...
    | cmd :: rest ->
        ...
        if String.length stripped = 0 then loop stack stack_env env rest
          ...
          | _ ->
              (* single command execution at function-level: use interpret_command for atomic ops *)
              let updated_stack, updated_stack_env, updated_env, stop_flag =
                (* interpret_command defined below; safe to call here since OCaml allows forward references *)
                (* We don't call the local wrapper; instead we'll call global interpret_command below by name. *)
                (* To avoid duplicating logic, we'll simply call interpret_command (global) here. *)
             ...
             (match env_lookup fname env with
              | Some (Closure (param, code, closure_env, is_inout)) ->
                  ...
                  if arg_value = Error then
                    ...
                    let fn_stack = [] in
                    (* Execute the entire function code using exec_commands *)
